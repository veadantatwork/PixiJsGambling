...
function loadUI(assetpath,x,y,clickval){
	...
	sprite.x = x;
	sprite.y = y;
	sprite.width = sprite.width*1.6;
	sprite.height = sprite.height*1.6;
	sprite.pivot.x = 0.5;
	sprite.pivot.y = 0.5;
...
function Rect2RectIntersection(ax,ay,adx,ady,bx,by,bdx,bdy){
	...
	bx -= bdx/2;
	by += bdy/2;
	// console.log("["+ax.toFixed(2)+", "+ay.toFixed(2)+", "+adx.toFixed(2)+", "+ady.toFixed(2)+"] ["+bx.toFixed(2)+", "+by.toFixed(2)+", "+bdx.toFixed(2)+", "+bdy.toFixed(2)+"]");
	// console.log((ax+adx > bx)+", "+ (ay-ady < by)+", "+ (bx+bdx > ax)+", "+ (by-bdy< ay));
